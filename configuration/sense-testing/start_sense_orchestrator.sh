...
docker run -p $SENSE_PORT_ONE:$SENSE_PORT_ONE -p $SENSE_PORT_TWO:$SENSE_PORT_TWO -e KC_SERVER=$SENSE_KC_SERVER --name $SENSE_CONTAINER_NAME --network $SOX_DOCKER_NETWORK -i -t docker.io/mail2xiyang/stackv
